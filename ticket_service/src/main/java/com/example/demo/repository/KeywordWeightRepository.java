...
public interface KeywordWeightRepository extends JpaRepository<KeywordWeight, Long> {
    ...
    @Query("SELECT kw FROM KeywordWeight kw WHERE kw.categoryName = :categoryName " +
            "AND LOWER(kw.keyword) LIKE LOWER(CONCAT('%', :text, '%'))")
    List<KeywordWeight> findByCategoryNameAndKeywordContainingIgnoreCase(
            @Param("categoryName") String categoryName,
            @Param("text") String text
    );

    Optional<KeywordWeight> findByCategoryNameAndKeyword(String categoryName, String keyword);
}
